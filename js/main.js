...
-------------------------------------------------------------------*/
  ...
  window.onload = function () {
    ...
    if (App.config.ajax.enabled) { // Set ajax.enabled property to false here
      PJAX.init();
    }
        ...
        request.onreadystatechange = function () {
          setTimeout(() => {
            if (this.readyState == 4 && this.status == 200) {
              ...
              formAlert.querySelector(
                ".ajax-form-alert__content"
              ).innerHTML = form.getAttribute("data-message-success");
            } else {
              formAlert.classList.add("is-active");
              formAlert.classList.add("is-error");
              formAlert.querySelector(
                ".ajax-form-alert__content"
              ).innerHTML = form.getAttribute("data-message-error");
            }
          }, 400);
        };
